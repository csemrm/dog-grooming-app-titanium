 ...
 *
 * @class Widgets.com.csemrm.navigationBar
 */

/**
 * @member Widgets.com.csemrm.navigationBar
...
if (CONFIG.image) {
    ...
    $.title = Ti.UI.createLabel({
        ...
        textAlign : CONFIG.textAlign ? CONFIG.textAlign : "center",
...
$.showRight = function(_params) {
    if (_params && typeof _params.callback !== "undefined") {
        ...
        if (_params.image) {
            $.fa.add($.rightImage, _params.image);
        } else if (_params.icon) {
            $.rightImage.backgroundImage = _params.icon;
        }
...
$.showNext = function(_callback) {
    if (_callback && typeof _callback !== "undefined") {
        ...
        $.next.visible = true;
        $.fa.add($.nextImage, 'fa-filter');
        $.next.addEventListener("click", _callback);
    }
};
/**
 * Shows the filter button
 * @param {Function} _callback The function to run on filter button press
 */
$.showFilter = function(_callback) {
    if (_callback && typeof _callback !== "undefined") {
        $.fa.add($.nextImage, 'fa-filter');
        $.nextImage.setAccessibilityLabel("filter");
        $.next.visible = true;
...
$.showSearch = function(_callback) {
    if (_callback && typeof _callback !== "undefined") {
        ...
        $.rightImage.setAccessibilityLabel("Search");
    }
};
 ...
 * @param {Function} _callback The function to run on action button press
 */
$.showMyRight = function(param) {
    var _callback = param._callback;
...
$.showMyNext = function(param) {
    ...
    if (_callback && typeof _callback !== "undefined") {
        if (image) {
            $.fa.add($.nextImage, image);
        } else if (icon) {

            $.nextImage.backgroundImage = icon;
        }
